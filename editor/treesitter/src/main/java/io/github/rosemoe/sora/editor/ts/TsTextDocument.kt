...
class TsTextDocument(language: TSLanguage) : AutoCloseable {
  ...
  internal fun doMod(mod: TextMod) {
    ...
    if (newText == null) {
      val startByte = edit.startByte.coerceAtLeast(0)
      val endByte = edit.oldEndByte.coerceAtLeast(startByte)
      val textByteLength = text.length shl 1
      if (startByte > textByteLength) {
        return
      }
      text.deleteBytes(startByte, endByte.coerceAtMost(textByteLength))
    } else {
      val safeStart = mod.start.coerceIn(0, text.length)
      if (safeStart == text.length) {
        text.append(newText)
      } else {
        text.insert(safeStart, newText)
      }
    }
