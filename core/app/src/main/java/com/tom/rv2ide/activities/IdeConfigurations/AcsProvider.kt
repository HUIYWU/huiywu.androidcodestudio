...
object AcsCommandInterface {
  ...
  class AcsCommandBuilder {
    ...
    private var splitUrls: List<String> = emptyList();
    ...
    private var enableSplitDownload: Boolean = false
    ...
    fun downloadFromSplitUrls(urls: List<String>): AcsCommandBuilder {
        this.splitUrls = urls
        return this
    }
    ...
    fun enableSplitDownload(splitUrls: List<String>): AcsCommandBuilder {
        this.enableSplitDownload = true
        this.splitUrls = splitUrls
        return this
    }
    ...
    fun execute(): AcsResult {
      return try {
        ...
        // if (splitUrls != null && splitUrls.isNotEmpty()) {
            // return AcsResult(success = true, output = splitUrls, errorOutput = "", exitCode = 0)
        // }
        ...
        try {
          val config =
              ACSConfig(
                  ...
                  splitUrls = splitUrls,
                  shouldDownload = true,
                  enableSplitDownload = splitUrls.isNotEmpty()
              )
        val file =
...
object AcsProvider {
  ...
  const val REPO_HOST = "gitee.com"//中国仓库源
  const val REPO_OWNER = "huiywu"
  const val REPO_NAME = "acs"
  const val ACS_BUILD_SYSTEM_REPONAME = "acs-ndk"
  ...
  val getManifestUrl: String = "${ACS_BUILD_SYSTEM_REPOURL}/raw/master/manifest.json"//中国仓库源
