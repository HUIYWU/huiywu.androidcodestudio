...
internal class DebuggingPreferences(
  ...
  init {
    ...
    addPreference(IdeLogsPreference())
    addPreference(IdeDebugLogsPreference())
  }
}
...
@Parcelize
internal class IdeLogsPreference(
...
@Parcelize
internal class IdeDebugLogsPreference(
    override val key: String = DevOpsPreferences.KEY_DEVOPTS_DEBUGGING_IDE_DEBUG_LOGS,
    override val title: Int = string.title_enable_ide_debug_logs,
    override val summary: Int? = string.msg_enable_ide_debug_logs,
) : SwitchPreference() {

  override fun onCreatePreference(context: Context): Preference {
    val pref = super.onCreatePreference(context)
    if (pref is androidx.preference.SwitchPreference) {
      pref.isChecked = DevOpsPreferences.ideDebugLogsEnabled
    }
    return pref
  }

  override fun onPreferenceChanged(preference: Preference, newValue: Any?): Boolean {
    DevOpsPreferences.ideDebugLogsEnabled = newValue as? Boolean ?: DevOpsPreferences.ideDebugLogsEnabled
    return true
  }
}
