...
class MoveFileTool(private val isPathAllowed: (String) -> Boolean) : AgentTool {

    override val spec = AgentToolSpec(
        name = NAME,
        description = "Move or rename a file or directory. Fails when the destination already exists; create its parent directory first when missing.",
            ...
            put(
                ...
                JSONObject().apply {
                    put(
                        ...
                        JSONObject().apply {
                            put("type", "string")
                            put("description", "Absolute path of the file or directory to move.")
                        }
                    )
                    put(
                        "destination",
                        JSONObject().apply {
                            put("type", "string")
                            put("description", "Absolute path to move it to.")
                        }
                    )
                }
            )
    ...
    override suspend fun execute(arguments: String): AgentToolResult {
        ...
        return if (sourceFile.renameTo(destinationFile)) {
            AgentToolResult("Moved: $source -> $destination")
        } else {
            AgentToolResult("Failed to move: $source -> $destination", isError = true)
        }
    }
