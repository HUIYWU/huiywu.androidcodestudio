...
class ProjectCreationCapabilitiesModelBuilder : ToolingModelBuilder {
  ...
  private fun candidatesFor(project: Project): List<ModuleCreationCandidate> {
    ...
    val candidates = mutableListOf<ModuleCreationCandidate>()

    if (project.pluginManager.hasPlugin(LIBRARY_PLUGIN)) {
      candidates.add(candidate(project, ModuleCreationKind.ANDROID_LIBRARY, ModuleSourceLanguage.JAVA, dsl, style))
      if (project.pluginManager.hasPlugin("org.jetbrains.kotlin.android") ||
          project.pluginManager.hasPlugin("kotlin-android")) {
        candidates.add(candidate(project, ModuleCreationKind.ANDROID_LIBRARY, ModuleSourceLanguage.KOTLIN, dsl, style))
      }
    }
    if (project.pluginManager.hasPlugin("java-library")) {
      candidates.add(candidate(project, ModuleCreationKind.JAVA_LIBRARY, ModuleSourceLanguage.JAVA, dsl, style))
      if (project.pluginManager.hasPlugin("org.jetbrains.kotlin.jvm") ||
          project.pluginManager.hasPlugin("kotlin")) {
        candidates.add(candidate(project, ModuleCreationKind.JAVA_LIBRARY, ModuleSourceLanguage.KOTLIN, dsl, style))
      }
    }
    return candidates
  }
  ...
  private fun candidate(
      ...
      language: ModuleSourceLanguage,
      dsl: GradleDsl,
      style: PluginApplicationStyle,
  ...
  private fun detectSettingsDsl(rootDir: File): GradleDsl =
      when {
