...
import com.tom.rv2ide.utils.Environment.BIN_DIR as BINDIR
...
class ACSProvider(
  ...
  private fun parsePackageEntry(jsonObject: JsonObject): PackageEntry {
    return PackageEntry(
        ...
        splitUrls =
        if (jsonObject.has("split_url"))
        jsonObject.getAsJsonArray("split_url").map {it.asString}
        else null,  //为分片提供
  ...
  suspend fun getField(config: ACSConfig): String =
      withContext(Dispatchers.IO) {
        ...
        when (config.getField) {
          ...
          "split_url" -> entry.splitUrls?.joinToString("\u001F") ?: ""//分隔符连接
          "url" -> entry.url ?: ""
          "sha256" -> entry.sha256 ?: ""
  ...
  suspend fun downloadFile(
    ...
    private suspend fun downloadSplitFile(
        entry: PackageEntry, callback: DownloadCallback?
    ): File {
        val splitUrls: List<String>? = entry.splitUrls
        val partFiles = mutableListOf<File>()
    
        splitUrls?.forEachIndexed { index, url ->
            val partFile = File(downloadDir, "${entry.filename}${index}")
            downloadFile(url, partFile, callback)
            partFiles.add(partFile)
        }
        
        val catBinary = File(BINDIR, "cat")
        if (!catBinary.exists()) {
          throw IOException("cat binary not found at: ${catBinary.absolutePath}")
        }
        
        if (!catBinary.canExecute()) {
          throw IOException("cat binary is not executable: ${catBinary.absolutePath}")
        }
        
        val processBuilder =
            ProcessBuilder(
                "sh", "-c",
                "${catBinary.absolutePath} ${partFiles.joinToString(" ")} > ${downloadDir}/${entry.filename}"
            )
            
        val env = processBuilder.environment()//配置命令的工作环境
        ...
        val mergedFile = File(downloadDir, "${entry.filename}")
  ...
  suspend fun downloadPackage(config: ACSConfig, callback: DownloadCallback? = null): File =
      withContext(Dispatchers.IO) {
        ...
        val outputFile: File
        //分片处理
        if (entry.splitUrls != null && entry.splitUrls.isNotEmpty()) {
            outputFile = downloadSplitFile(entry, callback)
        } else {
            outputFile = File(downloadDir, entry.filename)
